...
### アンカー実装の突き合わせ(当プラグイン ⇄ 参照実測)

参照側の `g1gc/anchor_tick → bin/14/15 → place_anchor/charge_anchor` を読み直し、
実測(通常戦闘・difficulty 2)と当プラグイン実装を突き合わせた結果:

| 項目 | 参照実測/コード | 当プラグイン | 判定 |
|---|---|---|---|
| 発動ラング | **全戦闘ラング**(difficulty 2 で実測 54回) | `fights` ゲートで全ラング | ✅ |
| 設置→チャージ | **4t**(45/54) = `anchor_cd`(5/4/4/3/1/1) | INTERMEDIATE 200ms = 4t | ✅ |
| チャージ→爆発 | **4t**(最頻) = `charge_cd`(5/4/3/2/2/2) | INTERMEDIATE 200ms = 4t | ✅ |
| サイクル | **最小 12t** = 4+4+4 | 200+200+200ms = 12t | ✅ |
| 爆発の無敵時間ゲート | `g1gc/perfect/hurt0..3` は **すべて「difficulty=6 以外は即 return 1」** = ラング1〜5 では無敵時間を見ない | ラング6のみ無敵ゲート | ✅ |
| アンカー中のクリスタル | `place_anchor`/`charge_anchor` が `crystal_timer = charge_cd` を再装填 → 交互 | `ab.anchorStage() != 0` でクリスタル停止 | ✅ |

→ アンカーは**構造・ラダー・ゲート条件すべて一致**。残る差分は
「設置位置の選び方(浮き設置スキャン・defenceplace のグロウストーン)」と
実戦での使用頻度(参照は pearl 80% の中で 3.5% をアンカーが占める)で、
これは side-by-side 比較(`tools/compare_fights.py`)で詰める段階。

## 2026-09-15 (4): パール着地修正 → 数値突き合わせ(b7〜b25)

ヘッドレス計測の詳細は `docs/parity/plugin_headless_runs.md` の「2026-09-15 (4)」節。
要点だけ:

- **チェイスパールの着地探索が壊れていた**(`findPearlLanding` が「地面はあるが立ち位置が無い」
  列で失敗し、145 s で 34 回。失敗ごとに共有 `pearlcd` を +250 ms 押し戻すため
  チェイスが飢餓 → 戻りが徒歩 3 s になり 6-9 帯が膨らんでいた)。着地を
  「レイ最初の固体地面 → 上に登って 2 ブロック空き」に直して解消。
- **パッシブは gap_timer(リンゴ咀嚼)より上で動かす**。map の `crystal/tick` の順序どおりで、
  咀嚼 1 回 1.75 s の間ずっと逃走パールが撃てなかったのが「3-4 ブロックで 4 s 立ち尽くし」の原因。
- **クリスタルはアンカー連鎖 3 回につき 1 回**(参照の武器比 18.1:5.3 = 1:3.4)。
  それまでは射程内でアンカー間隙ごとに発火し 13.7-14.1/min と 2.6 倍だった。

最終(b24 = 1.76.25)は パール 55.4/min ✓、クリスタル 5.0/min ✓、剣 8.3/min ✓、
滞在 (3,6] 38.3 % ✓・>9 53.5 % ✓・(6,9] 1.9 % ✓。残差は ≤2 帯(参照 4.6 % / 当側 0.0 %)、
(2,3] 帯(3.5 / 6.3)、アンカー設置 22.3/min(参照 18.1)。
