...
## 内容包絡ガード（輸送の証明に加え「内容の合法性」の証明強化）

輸送層（ダイジェスト束縛）は「通信が改竄されていない」ことを証明しますが、
**tag を通過したコンテンツそのものの合法性**は別問題です。これを
物理包絡の決定的チェックで補完します（Grimの方式に倣い「不可能なもののみ」判定）：

- **リーチ包絡**（`EnvelopeGuardListener`）: バニラの対エンティティリーチは3.0ブロック。
  視点→ヒットボックス**中心**の距離がフラット上限4.6超なら、疾走/跳躍/最悪ラグでも
  物理的に不可能 → ヒットを無効化＋必須ユーザーはキック（`kick-impossible`）。
  上限は中心測定+合法拡張充分マージン込みなので誤検出ゼロ。乗物搭乗中の攻撃は
  判定対象外（フラット上限の安全性証明が及ばないため）。
- セッション中のjar差し替え検出: Mod側は mtime/サイズ番兵でハッシュを再計算し、
  ハートビート毎に最新値を報告（サーバーは都度ピンと照合）。
- 移動ダイジェスト側の整合性修正: 他プラグインがmoveイベントをキャンセルしても
  **ダイジェストには必ず記録**する（通信ストリーム＝真実、ゲーム状態への受理＝
  別層、という一貫ルール）。

## 形式的主張（証明可能性の輪郭）

脅威モデル: 攻撃者は任意コードを実行できるが、TCPチャネルは順序保存・欠損なし、
サーバー側は正直（Dolev-Yao近似）。このとき:

1. **輸送健全性**: サーバーが必要ユーザーとして受理を継続する ⟺
   `digest(クライアントtap系列) == digest(サーバー受信系列)` が成立し続ける。
   不一致はリシンククォータ（3回/10分）以内で必ず枯渇しキック。
   ⇒ 受理されたセッションでサーバーが観測した動作は、証明済みModを
   通過した動作と**ビット等価**。
2. **オラクル抑止**: クリーンな分身で手続きを踏むには、チート通信と同一の
   移動系列を分身が生成する必要がある（=分身がチートする）— 矛盾。
3. **内容包絡**: 輸送が証明済みでも、不可能動作（リーチ包絡など、今後拡張可能）を
   内容が含む瞬間に無効化・必要ユーザーをキック。
4. **普遍天井**: 同一JVMで tap の計算根まで一貫して掌握する攻撃は、
   同一権限レベルでは原理上閉じられません（権限分離=カーネル/TEEが必要）。
   ただしその場合でも 1) の等式が必要条件に留まり、**サーバーが見る動作は
   tapに記録された動作と一致する限り受理される**ため、攻撃者は「サーバーに
   合法的に見える系列」を維持するしかなく、輪郭外の優位は包絡ガード側の領分として
   段階的に拡張して塞ぎます。
