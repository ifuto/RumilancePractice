...
execute at @s[scores={OnGround=1}] if score @p[tag=xlib_target] in_cobweb_decision matches 1 unless entity @e[type=arrow,nbt={inGround:0b}] at @p[tag=xlib_target,distance=..7] facing entity @s feet rotated ~ 0.0 run fill ^2 ^1 ^2 ^-2 ^-1 ^ command_block{auto:1b,Command:"function quantum:cart/mark"} replace #xaniclelib:nonsolid
execute at @s[scores={OnGround=1}] unless score @s hit_decision_without_cd matches 1 unless score @s crit_decision_without_cd matches 1 unless entity @e[type=arrow,nbt={inGround:0b}] at @p[tag=xlib_target,distance=..7] facing entity @s feet rotated ~ 0.0 run fill ^2 ^1 ^2 ^-2 ^-1 ^ command_block{auto:1b,Command:"function quantum:cart/mark"} replace #xaniclelib:nonsolid
