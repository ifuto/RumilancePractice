scoreboard players set .mode mode 602
title @a times 2 5 2
title @a title {"text":"-2","color": "gold"}
execute as @a[tag=xlib_bot] at @s run function mech_train:crystal/common_init
execute at @p[tag=xlib_target] run fill ~-1 ~ ~-1 ~1 ~1 ~1 netherite_block replace air
execute as @a[tag=xlib_target] at @s positioned over world_surface run tp @s ~ ~ ~ facing ~ ~-2 ~3
attribute @a[tag=xlib_bot] max_health base set 10
...
execute as @a[tag=xlib_bot] at @s run function mech_train:cart/-2/loop