...
public final class DuelCommand implements CommandExecutor, TabCompleter {
    ...
    public void handleCancel(Player player) {
        java.util.Optional<DuelRequestService.RichDuelRequest> outgoing =
                duelRequestService.latestOutgoing(player.getUniqueId());
        if (outgoing.isPresent()) {
            DuelRequestService.RichDuelRequest req = outgoing.get();
                ...
                .ifPresentOrElse(req -> {
                    ...
                    target.sendMessage(messageService.render(targetLocale, "duel.request-received",
                            ...
                            .append(Component.text("[ACCEPT]", NamedTextColor.GREEN)
                                    .decorate(TextDecoration.BOLD)
                                    .clickEvent(ClickEvent.runCommand("/rpaccept " + sender.getName())))
                            .append(Component.space())
                            .append(Component.text("[DENY]", NamedTextColor.RED)
                                    .decorate(TextDecoration.BOLD)
                                    .clickEvent(ClickEvent.runCommand("/rpdeny " + sender.getName()))));
