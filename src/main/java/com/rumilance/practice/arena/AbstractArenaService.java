...
public abstract class AbstractArenaService implements ArenaService {
    ...
    @Override
    public boolean reassignInstance(UUID instanceId, UUID newMatchId) {
        if (instanceId == null || newMatchId == null) {
            return false;
        }
        reservationLock.lock();
        try {
            for (ArenaInstance instance : instancesByTemplate.values()) {
                if (instance.id().equals(instanceId)) {
                    instance.assignMatch(newMatchId);
                    return true;
                }
            }
            return false;
