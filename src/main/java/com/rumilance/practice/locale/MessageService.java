...
public final class MessageService {
    ...
    /** Renders {@code key} in {@code player}'s resolved locale. */
    public Component render(Player player, String key, TagResolver... resolvers) {
        return render(resolveLocale(player), key, resolvers);
    }

    /** Raw MiniMessage string for {@code key} in {@code player}'s locale (no deserialize). */
    public String raw(Player player, String key) {
        return localeService.rawMessage(resolveLocale(player), key);
    }
