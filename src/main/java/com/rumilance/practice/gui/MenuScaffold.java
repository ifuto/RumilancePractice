...
public final class MenuScaffold {
    ...
    /** Fills every slot with the black background tile. Prefer {@link #chrome} for menus. */
    public static Inventory fillBackground(Inventory inventory) {
        ItemStack filler = ItemBuilder.hiddenFill();
    ...
    /**
     * Standard chrome: tooltip-less bars on the first and last rows only.
     * Content rows stay empty so icons read clearly.
     */
    public static Inventory chrome(Inventory inventory) {
        ItemStack fill = ItemBuilder.hiddenFill();
        int rows = rowCount(inventory);
        for (int col = 0; col < GuiSlots.ROW_SIZE; col++) {
            inventory.setItem(GuiSlots.slot(0, col), fill);
            if (rows >= 2) {
    ...
    /** Top bar only — for 5-row kit editor where the last row is player slots. */
    ...
    /**
     * Single bottom-centre control. Nested screens show Back; root screens show Close.
     * Action is always {@code close} so {@code GuiListener} can bounce nested menus to the parent.
     */
    public static Inventory nav(Inventory inventory, GuiSession session) {
        return nav(inventory, session, Component.text("Close", UiTheme.DANGER),
                Component.text("Back", UiTheme.WARNING));
    }

    public static Inventory nav(Inventory inventory, GuiSession session, Component closeLabel, Component backLabel) {
        boolean nested = session != null && (session.fromGameMenu() || session.fromBattleMenu());
        return closeButton(inventory, nested ? backLabel : closeLabel);
    }

    /** Single bottom-centre return control (replaces separate Back + Close). */
    public static Inventory returnButton(Inventory inventory) {
        return returnButton(inventory, Component.text("Back", UiTheme.WARNING));
    }

    public static Inventory returnButton(Inventory inventory, Component label) {
    ...
    /** Places a back button at the bottom-left. Prefer {@link #nav} for player menus. */
    ...
    public static void pagingButtons(Inventory inventory, int page, int totalItems) {
        pagingButtons(inventory, page, totalItems,
                Component.text("◀ Previous", UiTheme.PRIMARY),
                Component.text("Next ▶", UiTheme.PRIMARY),
                Component.text((page + 1) + " / " + Math.max(1, (totalItems + gridPageSize() - 1) / gridPageSize()),
                        UiTheme.MUTED),
