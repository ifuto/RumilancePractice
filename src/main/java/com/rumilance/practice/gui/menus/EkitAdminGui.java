...
public final class EkitAdminGui extends AbstractGui implements BottomInventoryClickHandler {
    ...
    protected Component title(Player player, GuiSession session) {
        ...
        if ("chest".equals(view)) {
            ...
            return t(player, "gui.ekit-admin-category",
                    com.rumilance.practice.locale.MessageService.tags("cat", cat == null ? "" : cat))
                    .color(UiTheme.PRIMARY);
        }
        return t(player, "gui.ekit-admin-title").color(UiTheme.PRIMARY);
    }
    ...
    private void renderChest(Player player, GuiSession session, Inventory inventory) {
        ...
        for (int i = 0; i < 45; i++) {
            if (i < entries.size()) {
                ...
                meta.lore(List.of(Component.text(line(player, "gui.ekit-admin-take"), UiTheme.MUTED)
                ...
                meta.displayName(Component.text(" ", UiTheme.MUTED)
                        .decoration(TextDecoration.ITALIC, false));
                meta.lore(List.of(Component.text(line(player, "gui.ekit-admin-drop"), UiTheme.MUTED)
                        .decoration(TextDecoration.ITALIC, false)));
    ...
    private boolean deposit(Player player, String cat, ItemStack item) {
        ...
        if (ekitItems.isPotionCategory(cat)) {
            ...
            if (effect == null) {
                player.sendMessage(t(player, "gui.ekit-admin-not-potion"));
                return false;
            }
