...
public final class PartyInviteGui extends AbstractGui {
    ...
    protected Component title(Player player, GuiSession session) {
        return Component.text("Invite Players", UiTheme.PRIMARY)
    ...
    protected void render(Player player, GuiSession session, Inventory inventory) {
        ...
        if (team == null || !team.isOwner(player.getUniqueId())) {
            inventory.setItem(MenuScaffold.gridSlot(13),
                    ItemBuilder.of(Material.BARRIER)
                            .name(Component.text("Not party owner", UiTheme.DANGER))
                            .action("decorate")
                            .build());
        ...
        for (int i = from; i < to; i++) {
            ...
            inventory.setItem(MenuScaffold.gridSlot(index++), skull(target));
        }
        if (candidates.isEmpty()) {
            inventory.setItem(MenuScaffold.gridSlot(13),
                    ItemBuilder.of(Material.BARRIER)
                            .name(Component.text("No players online", UiTheme.MUTED))
                            .action("decorate")
                            .build());
        }
        MenuScaffold.pagingButtons(inventory, page, candidates.size());
        MenuScaffold.backButton(inventory);
        MenuScaffold.closeButton(inventory);
    }
