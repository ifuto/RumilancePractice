...
public final class PotionGui extends AbstractGui {
    ...
    protected Component title(Player player, GuiSession session) {
        return t(player, "gui.potion-title").color(UiTheme.PRIMARY);
    }
                ...
                for (boolean extended : extendedOptions) {
                    ...
                    meta.displayName(Component.text(
                                    PotionRules.variantLabel(variant) + " " + opt.display()
                                            + (opt.maxLevel() > 1 ? " " + line(player, "gui.level") + " " + roman(level) : "")
                                            + (extended ? " " + line(player, "gui.extended") : ""),
                                    UiTheme.VALUE)
                            .decoration(TextDecoration.ITALIC, false));
                    meta.lore(List.of(Component.text(line(player, "gui.potion-add"), UiTheme.MUTED)
                            .decoration(TextDecoration.ITALIC, false)));
