...
public final class KitArenaSelectGui extends AbstractGui {
    ...
    protected Component title(Player player, GuiSession session) {
        ...
        return Component.text("Arenas: " + (kit == null ? "?" : kit), UiTheme.PRIMARY)
    ...
    protected void render(Player player, GuiSession session, Inventory inventory) {
        ...
        inventory.setItem(GuiSlots.slot(0, 4), ItemBuilder.of(Material.BOOK)
                .name(Component.text("デュエル左 / パーティ右", UiTheme.SECONDARY))
                .lore(
                        UiTheme.line("空 = ランダム"),
                        UiTheme.line("緑 = 選択中")
                )
        ...
        for (ArenaTemplate t : templates) {
            ...
            inventory.setItem(GuiSlots.slot(row, col), ItemBuilder.of(duelMat)
                    .name(Component.text("Duel: " + label, inDuel ? UiTheme.SUCCESS : UiTheme.MUTED))
                    .lore(UiTheme.hint("左クリックでトグル"))
                    .action("duel:" + t.name())
                    .build());
            inventory.setItem(GuiSlots.slot(row, col + 5), ItemBuilder.of(partyMat)
                    .name(Component.text("Party: " + label, inParty ? UiTheme.SUCCESS : UiTheme.MUTED))
                    .lore(UiTheme.hint("左クリックでトグル"))
                    .action("party:" + t.name())
                    .build());
